...
#antara dua media fermentasi (gula vs hidrolisat lignoselulosa) 
#Pada modul ini kita menggunakan pendekatan statistik limma (Linear Models 
#for Microarray Data), yang merupakan standar untuk data microarray.
...
#illuminaHumanv4.db: database anotasi khusus untuk chip Illumina (GSE218764) 
...
gset <- getGEO("GSE218764", GSEMatrix = TRUE, AnnotGPL = TRUE)[[1]]
...
#pData(): metadata sampel 
#Kita mengambil kolom ‘source_name_ch1’ yang berisi info vaksin vs baseline 
group_info <- pData(gset)[["source_name_ch1"]] 

#melihat jumlah sampel 9h karena data yang dibandingkan hydrolysate (9h) dan 
#sugars (9h) untuk melihat pengaruh media yang digunakan
table(pData(gset)$source_name_ch1)

#filter hanya 9h
keep <- grepl("9h", pData(gset)$source_name_ch1)

gset_9h <- gset[, keep]

#Membuat grub baru dari gset_9h
group_info <- ifelse(
  grepl("hydrolysate", pData(gset_9h)$source_name_ch1),
  "hydrolysate",
  "sugars"
)

gset_9h$group <- factor(group_info)
...
gset_9h$group <- factor(groups) 

#levels(): melihat kategori unik dalam faktor 
nama_grup <- levels(gset_9h$group) 
print(nama_grup) 
...
design <- model.matrix(~0 + gset_9h$group) 

#colnames(): memberi nama kolom agar mudah dibaca 
colnames(design) <- levels(gset_9h$group) 

#Menentukan perbandingan biologis 
grup_hydro <- "hydrolysate"
grup_sugar <- "sugars"

contrast_formula <- paste(grup_hydro, "-", grup_sugar) 
...
#Cek platform dataset
annotation(gset_9h)
#Jenis platform dataset GPL2529 dalah platform spesifik untuk yeast

#Install BiocManager untuk annotation package yeast2.db
...
#Cek hasil anotasi 
head(topTableResults[, c("PROBEID", "COMMON", "GENENAME")])
...
group_colors <- as.numeric(gset_9h$group)
...
expr_long <- data.frame( 
  Expression = as.vector(ex_9h), 
  Group = rep(gset_9h$group, each = nrow(ex_9h)) 
) 
...
ggplot(expr_long, aes(x = Expression, color = Group)) + 
  ...
  theme_minimal() + 
  labs( 
    title = "Distribusi Nilai Ekspresi Gen (hydrolysate)", 
...
ggplot(umap_df, aes(x = UMAP1, y = UMAP2, color = Group)) + 
  ...
  theme_minimal() + 
  labs( 
    title = "UMAP Plot: Sugars vs Hydrolysate", 
    x = "UMAP 1", 
    y = "UMAP 2" 
  )
...
ggplot(volcano_data, aes(x = logFC, y = -log10(adj.P.Val), color = 
  ...
  theme_minimal() + 
  ggtitle("Volcano Plot DEG Hydrolysate") 
...
mat_heatmap <- ex_9h[top50$PROBEID, ]

#Gunakan Gene Symbol (fallback ke Probe ID) 
gene_label <- ifelse( 
  is.na(top50$COMMON) | top50$COMMON == "", 
  top50$PROBEID,      
  # jika COMMON kosong → probe ID 
  top50$COMMON        
) 
...
write.csv(topTableResults, "Hasil_GSE218764_DEG.csv") 

message("Analisis selesai. File hasil telah disimpan.") 

#PART L. GO ENRICHMENT & KEGG PATHWAY ENRICHMENT
...
BiocManager::install("org.Sc.sgd.db")   #untuk yeast
...
gene_list <- unique(deg_sig$SGD)

#Konversi KE ENTREZID
#
gene_df <- bitr(
  gene_list,
  fromType = "SGD", 
...
#Visualisasi KEGG Pathway Enrichment
